...
5. Every city is within one of the submitted countries (${countriesList}) — unless the traveler's notes explicitly requested a city elsewhere. On multi-country trips, cities are grouped by country with NO backtracking into a country already left — walk the cities list in order and confirm each country's cities form one unbroken block. Concretely: Hong Kong (China) → Osaka/Kyoto/Tokyo (Japan) → Seoul (Korea) → Xi'an/Shanghai (China) is WRONG — China appears, gets left, then reappears at the end. The correct fix is to move China's cities together, e.g. Osaka/Kyoto/Tokyo (Japan) → Seoul (Korea) → Xi'an/Shanghai/Hong Kong (China), or open and close in China if that better matches the arrival/departure airports.
...
8. Every MID-ROUTE city (not the arrival or departure city, which are exempt) has at least its size-classification minimum nights from Step 2 — Micro/Small can legitimately be 1 night, but Medium/Large/Mega cities (including famous single-attraction destinations like Xi'an, Petra, or Machu Picchu's gateway city) must NOT be compressed to 1 night just to fit more stops. A 1-night stop should have enough time for its actual headline attraction, not just a fallback walk. If a must-visit mid-route city can't get enough nights within the trip length, CUT it entirely rather than under-nighting it.
